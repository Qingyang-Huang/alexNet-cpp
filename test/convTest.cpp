...
TEST(ConvolutionalLayer, Conv2D) {
    ...
    ConvolutionalLayer convLayer(3, 3, 2, 2, 1, 1, 1, 1, 1); 
...
TEST(ConvolutionalLayerTest, Forward2) {
    cv::Mat input = cv::Mat(3, 9, CV_32F);

    cv::randu(input, cv::Scalar::all(1), cv::Scalar::all(10));

    ConvolutionalLayer *convLayer;
    convLayer = new ConvolutionalLayer(3, 3, 2, 2, 1, 1, 1, 3, 6); 
    // 调用forward方法
    convLayer->forward(input);
...
TEST(ConvolutionalLayer, TransConv2D) {
    cv::Mat input = (cv::Mat_<float>(1, 3*3) << 1, 2, 3,
                                             4, 5, 6,
                                             7, 8, 9);
    cv::Mat kernel = (cv::Mat_<float>(2, 2) << 1, 0,
    ...
    ConvolutionalLayer convLayer(4, 4, 2, 2, 2, 2, 1, 1, 1); 
...
TEST(ConvolutionalLayerTest, BackWard) {
    cv::Mat input = cv::Mat(3, 9, CV_32F);
