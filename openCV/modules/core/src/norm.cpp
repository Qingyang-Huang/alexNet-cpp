...
double norm( InputArray _src, int normType, InputArray _mask )
{
    ...
    if(_src.dims() == 2) {
        double scale;
        if(mask.empty()) {
            CALL_HAL_RET(norm1, cv_hal_norm1, scale, src.ptr(), src.step, CV_MAT_DEPTH(src.type()),
                        NULL, 0, 0, src.cols, src.rows, CV_MAT_CN(src.type()), normType);
        }
        else {
            CALL_HAL_RET(norm1, cv_hal_norm1, scale, src.ptr(), src.step, CV_MAT_DEPTH(src.type()),
                        mask.ptr(), mask.step, CV_MAT_DEPTH(mask.type()), src.cols, src.rows, CV_MAT_CN(src.type()), normType);
        }
    }
