                    ...
                    Point* minLoc, Point* maxLoc, InputArray mask )
{
    ...
    if(_img.dims() == 2) {
        Mat src = _img.getMat(), _mask = mask.getMat();
        if(mask.empty()) {
            CALL_HAL(minMaxLoc, cv_hal_minMaxLoc, src.ptr(), src.step, CV_MAT_DEPTH(src.type()),
                    NULL, 0, 0, src.cols, src.rows, CV_MAT_CN(src.type()), minVal, maxVal, (int*)minLoc, (int*)maxLoc);
        }
        else {
            CALL_HAL(minMaxLoc, cv_hal_minMaxLoc, src.ptr(), src.step, CV_MAT_DEPTH(src.type()), _mask.ptr(), _mask.step,
                    CV_MAT_DEPTH(_mask.type()), src.cols, src.rows, CV_MAT_CN(src.type()), minVal, maxVal, (int*)minLoc, (int*)maxLoc);
        }
    }
